...
- 当前后续任务：从 `platform/macos-bootstrap` 新建 `platform/process-adapter`，按 MAC-02 抽出跨平台进程终止能力；随后优先执行 MAC-03，修正 macOS Office COM unsupported 早退，避免触发 PowerShell；基础 CI 先按 MAC-07 落地，打包在资源配置平台化后再做 MAC-06。
...
- 当前 Git 分支：`platform/macos-bootstrap`，已包含 PR #1/#2/#3。
