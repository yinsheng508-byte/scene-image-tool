...
- 当前后续任务：从 `platform/macos-bootstrap` 新建 `platform/macos-export-preflight-ui`，按 MAC-04 修正 macOS 导出预检 UI 文案和交互；基础 CI 可按 MAC-07 并行落地，打包在资源配置平台化后再做 MAC-06。
...
- 当前 Git 分支：`platform/macos-bootstrap`，已包含 PR #1/#2/#3/#4/#5。
