...
- 当前后续任务：合并 `infra/github-desktop-ci`；随后进入 MAC-08 Mac PDF / 拼图 / 图片能力验收矩阵。
...
- 2026-08-28 MAC-07 已新增 `.github/workflows/desktop-ci.yml`，使用 Windows/macOS matrix、Node 22、npm cache、`npm ci`、`puzzle:shadow:smoke`、`puzzle:text:smoke` 和 `git diff --check`；第一版 CI 不调用 LibreOffice、字体探针、打包或发布脚本。PR #9 Actions 已通过：Windows job 45s，macOS job 37s。
