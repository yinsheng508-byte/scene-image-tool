...
1. MAC-01 至 MAC-09 已完成并进入 `platform/macos-bootstrap`：adapter 总壳、进程 adapter、统一 capability/health、Mac 导出预检 UI、LibreOffice fixture smoke、`dist:mac:dir`、基础 CI、渲染 QA 和设置页能力面板。
2. PR #11 已合并，Desktop CI 通过：Windows job 53s，macOS job 32s。
3. 下一步进入 MAC-10，优先做最小 service 层拆分，继续避免大范围业务重构。
