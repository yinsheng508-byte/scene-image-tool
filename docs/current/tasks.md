...
| GitHub 公开同步执行 | 本地已完成，待远端 | `docs/current/github-public-sync-runbook.md` | 干净导出仓库、公开远端推送指令、Mac clone 验收 | `D:\deptask\scene-image-tool-github-public` 已生成并提交，已创建 `platform/macos-bootstrap`；无 95 MiB+ 文件，无敏感文件路径；当前机器未安装 `gh`，还缺真实远端 URL |
| GitHub 同步与 macOS 准备 | 已推进 | `docs/current/github-sync-macos-plan.md` | GitHub 远端、历史大文件、资源外部化、macOS adapter | 公开同步路线已从“待决策”推进为“只公开干净导出仓库，不公开原历史” |
