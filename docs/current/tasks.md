...
| MAC-06：macOS unsigned app bundle | 已完成，待合并 | `code/desktop/package.json`、`code/desktop/assets/app-icon.icns` | `dist:mac:dir`、macOS `dir` target、`.icns`、平台化 `extraResources` | `dist:mac:dir` 成功生成 `dist/mac-arm64/流量蜂虚拟笔记工具.app`；app 10 秒启动 smoke 通过；包内无 Windows LibreOffice runtime / redist / PowerShell scripts；`dist/` ignored |
