...
| MAC-03：统一导出 capability / health 返回结构 | 已完成，待合并 | `code/desktop/platform/common/health-report.js`、`code/desktop/main.js`、`code/desktop/preload.js` | health normalizer、Office COM 非 Windows 早退、`capability:getAll` IPC | `export:healthCheck`/`office:healthCheck` 保留旧字段并新增 `platform/engine/capability/capabilities/errorCode/message`；macOS Office COM 返回 `PLATFORM_UNSUPPORTED`，不进入 PowerShell；preload 暴露 `getCapabilities` |
