...
| MAC-02：跨平台进程终止 adapter | 已完成，待合并 | `code/desktop/platform/common/process-utils.js`、`code/desktop/platform/darwin/process-tree.js`、`code/desktop/platform/win32/process-tree.js` | 导出取消进程终止边界、`main.js` adapter 调用 | macOS 取消不调用 `taskkill`；Windows adapter 保留 `taskkill /T /F`；非 Windows `getProcessNameByPid` 不进入 PowerShell；node check、adapter 直接调用和 puzzle smoke 通过 |
