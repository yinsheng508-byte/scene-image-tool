...
| MAC-12 macOS signing release 准备 | `node --check`、plist lint、workflow YAML parse、missing-signing preflight、`dist:mac:dir` | 准备通过；真实签名未执行 | 本机 0 个有效 signing identities 且无 `CSC_*` / `APPLE_*` env；`signing:mac:check` 和 `dist:mac` 在缺凭据时阻断；unsigned `dist:mac:dir` 仍通过 |
