...
| MAC-10 settings service 拆分 | `node --check`、临时 userData service smoke、Electron 启动、两个 puzzle smoke | 通过 | `main.js` 从 9180 行降至 9105 行；`settings:getAll` / `settings:set` IPC contract 和 userData `app-settings.json` 路径保持不变 |
